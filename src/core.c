...
static void Core_execute(Core *self, inst_fields_t inst_fields, inst_enum_t inst_enum) {
    ...
    int32_t imm_s = SEXT((GETBITS(raw, 11, 7) | (GETBITS(raw, 31, 25) << 5)), 12);
    int32_t imm_b = SEXT(
    ...
    case OP: { // 0x33
        ...
        switch (funct3) {
        case 0x0: // ADD/SUB
            if (funct7 == 0x20) res = (reg_t)((int32_t)v1 - (int32_t)v2); // SUB
            else                 res = v1 + v2;                            // ADD
            break;
        case 0x1: // SLL
            res = v1 << (v2 & 31u);
